import os
os.chdir("D:\\image_processing_course")
...
unique_labels, counts = np.unique(ar_labels, return_counts=True)
...
# Compute confusion matrix. For 2x2 out put is TN, FP, FN, TP
df_cm = pd.DataFrame(confusion_matrix(ar_labels.tolist(), predictions.tolist(), labels = list(unique_labels)))
df_cm

cm_cr = classification_report(ar_labels.tolist(), predictions.tolist(), target_names = list(unique_labels), output_dict = True)
cm_cr

#Statistics are also available as follows
print("Overall Accuracy is ", round(accuracy_score(ar_labels.tolist(), predictions.tolist()), 2),", Kappa is ", round(abs(cohen_kappa_score(ar_labels.tolist(), predictions.tolist())), 2))
#SVC: Overall Accuracy is  0.85 , Kappa is  0.83
...
param = {'n_estimators': 1000, # The number of trees in the forest.
         ...
         'max_depth': None, 'bootstrap': True, 'max_features': "sqrt", # The number of features to consider when looking for the best split
...
print("Overall Accuracy is ", round(accuracy_score(ar_labels.tolist(), predictions.tolist()), 2),", Kappa is ", round(abs(cohen_kappa_score(ar_labels.tolist(), predictions.tolist())), 2))
#SVC: Overall Accuracy is  0.85 , Kappa is  0.83
#RF: Overall Accuracy is  0.96 , Kappa is  0.96
